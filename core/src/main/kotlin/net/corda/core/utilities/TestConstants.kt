...
val ALICE: Party get() = Party(X500Name("CN=Alice,O=SmallCorp,OU=Accounts,L=London,C=UK"), ALICE_KEY.public)

val BOB_KEY: KeyPair by lazy { entropyToKeyPair(BigInteger.valueOf(80)) }
/** Dummy individual identity for tests and simulations */
val BOB: Party get() = Party(X500Name("CN=Bob,O=SmallCorp,OU=Accounts,L=London,C=UK"), BOB_KEY.public)

val CHARLIE_KEY: KeyPair by lazy { entropyToKeyPair(BigInteger.valueOf(90)) }
/** Dummy individual identity for tests and simulations */
val CHARLIE: Party get() = Party(X500Name("CN=Charlie,O=SmallCorp,OU=Accounts,L=London,C=UK"), BOB_KEY.public)
